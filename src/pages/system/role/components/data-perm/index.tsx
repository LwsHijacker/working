import {Button, Checkbox, Drawer, Form, Input, message, Select, Space, Spin, Tree} from "antd";
...
import {roleDataScope} from "@/api/role.ts";
...
const DataPermDrawer: React.FC<Role.RoleDataPermProp> = ({roleProp,visible, close}) => {
    ...
    const dataScope = useRequest(roleDataScope, {manual: true, onSuccess: () => {
        message.success('设置数据完成成功');
        close(false)
    }})
    ...
    const onCheck = (value: { checked: React.Key[]; halfChecked: React.Key[]; } | React.Key[]) => {
        ...
        setSelectKeys(keys);
        form.setFieldValue('dataValue', keys);
    };

    const changeDataScope = (e: number) => {
    ...
    useEffect(() => {
        if (visible) {
            form.setFieldsValue({roleId: roleProp.roleId, roleName: roleProp.roleName, roleCode: roleProp.roleKey, dataType: 1})
        }
        return () => {
            form.resetFields();
        }
    ...
    >
        <Spin tip="加载中......" spinning={dataScope.loading || loadTree.loading}>
            ...
            >
                ...
                <Form.Item name="dataType" label="数据范围" rules={[{ required: true, message: '请选择数据范围' }]}>
                    <Select
                        style={{ width: '100%' }}
                ...
                {form.getFieldValue('dataType') === 2 && <Form.Item label="数据权限" name="dataValue" rules={[
                    {required: false},
                    () => ({
                        validator(_, value: number[]) {
                            if (form.getFieldValue("dataType") === 2 && value.length <= 0) {

...
import {meetingSaveHelpInfo} from "@/api/meeting.ts";
...
const MeetingHelpForm = forwardRef<Ref, MeetingFormProps>((props:MeetingFormProps, ref) => {
    ...
    const {loading, run} = useRequest(meetingSaveHelpInfo, {
        manual: true,
        onSuccess: (_) => {
    ...
    return (
        ...
        >
            ...
            <Form labelCol={{span: 5}}
                ...
                <Form.Item label={"会议帮助内容"} name={"meetingHelper"} rules={[{required: true, message: '必填项'}]}>
                    <RichEditor defaultValue={richEditorDefaultValue}
                                onChange={(content) => form.setFieldValue("meetingHelper", content)}/>
                </Form.Item>
            </Form>
        </Drawer>
    )
...
export default MeetingHelpForm

...
import {Button, Card, message, Modal, Space, Switch, Table} from "antd";
...
const SystemDictPage = () => {
    ...
    const columns: ColumnsType<DictPageProp> = [
        ...
        },
        {
            ...
            dataIndex: 'dictType',
            align: 'center',
            render: (_, record) => <Button size='small' type="link">{record.dictType}</Button>
        },
        {
            title: '状态',
            key: 'status',
            ...
            align: 'center',
            width: 160,
            render: (_, record) => <Switch size='small' checkedChildren="正常" unCheckedChildren="停用" checked={record.status === 1} />
        },
        {
            title: '备注',
            key: 'remark',
    ...
    return <Container>
        <Card className="dict-card-top">
            <Space>
                ...
                <Button type="primary" danger icon={<DeleteOutlined />} onClick={() => deleteDictHandler()}>删除</Button>
            </Space>
        </Card>

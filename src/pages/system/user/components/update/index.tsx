...
const UserUpdateDrawer: React.FC<User.UserUpdateDrawerProp> = ({visible, userId, close}) => {


    const [form] = Form.useForm<User.UserUpdateFormProp>();
    ...
    >
        <Spin tip="加载中......" spinning={updateHandler.loading}>
            ...
            >
                ...
                <Form.Item name="phone" label="手机号码" rules={[{validator: User.validateMobile, required: false, message: '请输入正确的手机号码'}]}>
                    <Input placeholder="请输入手机号码" />
                </Form.Item>
                <Form.Item name="email" label="邮箱" rules={[{validator: User.validateEmail, required: false, message: '请输入正确的邮箱格式'}]}>

...
import './index.less';
import meentImg from "@/assets/images/meeting.png"
...
const LayoutSider: React.FC<LayoutHeaderProp> = ({collapsed}) => {
    ...
    const routerBuildMenu = (data: Menus.MenuItemProp[]): MenuItem[] => {
        return data.map((item) => {
            ...
            const key = item.path && item.path !== "" ? item.path : item.perms;
    ...
    useEffect(() => {
        setMenuList([...routerBuildMenu(data.menus)]);
    }, []);
    ...
    return <Layout.Sider width={230} trigger={null} collapsible collapsed={collapsed}>
        <div className="ant-pro-sider-logo">
            <a href="">
                <img src={meentImg} alt="logo"/>
                {!collapsed && <h1>会议管理系统</h1>}
            </a>
        </div>
        <Menu
            mode="inline"

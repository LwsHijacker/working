...
export const roleDataScope = (param: Role.DataPermFormProp): Promise<string> => {
    return https.request({
        url: '/role/dataAuth',
        method: 'post',
        params: param,
...
// roleChangeStatus 角色状态修改
export const roleChangeStatus = (roleId: number, status: boolean): Promise<string> => {
    return https.request({
        url: '/role/status',
        method: 'post',
        params: {roleId: roleId, status: status ? 1 : 0}
    })
}
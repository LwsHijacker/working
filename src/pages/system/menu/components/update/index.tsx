...
const MenuUpdateDrawer: React.FC<Menus.MenuUpdateDrawerProp> = ({visible, menuId, close}) => {

    const [menuType, setMenuType] = useState<number>(1);
    const [form] = Form.useForm<Menus.MenuUpdateFormProp>();
    const [tree, setTree] = useState<Menus.MenuTreeProp[]>([]);
        ...
        <Spin tip="加载中......" spinning={loadInfo.loading || loadTree.loading}>
            <Form labelCol={{ span: 5 }}
                ...
                <Form.Item name="menuCode" label="菜单编码"  rules={[{ required: true, message: '请输入菜单编码' }]} >
                    <Input placeholder="请输入菜单编码" readOnly />
                </Form.Item>
